...
class ShowProductScreen extends StatefulWidget {
  ...
  const ShowProductScreen({Key key, this.isPurchaseMode = false})
      : super(key: key);
  @override
      ...
      child: MolButton(context).blueButton(
          onPressed: () {
            if (!widget.isPurchaseMode)
              Navigator.pushNamed(context, AppRoute.MANIPULATE_PRODUCT_ROUTE,
                  arguments: {"isEdit": true});
            else {
              Navigator.pushNamed(context, AppRoute.PURCHASE_ROUTE);
            }
          },
          child: Text(
            widget.isPurchaseMode ? "Beli" : "Ubah",
